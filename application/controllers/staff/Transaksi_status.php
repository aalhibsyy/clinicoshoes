...
class Transaksi_status extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $status = $this->mod_status->staff_get_limit_data($config['per_page'], $start, $q);
    ...
    public function update($id)
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('staff/transaksi_status/update_action'),
            ...
            $this->template->load('template/backend/dashboard', 'staff/transaksi/update_form', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('staff/transaksi_status'));
        }
    }
